...
Protocol 5.11 is a backward-compatible **tool-assisted engineering methodology and capability refinement**. It preserves the Protocol 5 hierarchy, two-role lifecycle, Protocol 5.9 routing/distribution architecture, Protocol 5.10 snapshot-complete handoffs, and all Protocol 5.4-5.10 hardening semantics unchanged while adding optional capability-aware guidance for Serena semantic repository work, Semgrep structural/variant analysis, and Hypothesis property/stateful testing. These tools remain evidence and development instruments rather than generic dependencies, lifecycle gates, normative task authority, or substitutes for affected regression and real-boundary integration.
...
Protocol 5.13 is a backward-compatible **deterministic tool-entry, CodeQL, and progressive-disclosure compression refinement**. It preserves the Protocol 5 hierarchy, two-role lifecycle, all Protocol 5.4-5.12 engineering/acceptance/convergence safeguards, and optional-tool status while making specialized routing operational per material engineering question. Symbol relations route directly to Serena methodology, structural relations to Semgrep, broad Python property/state relations to Hypothesis, and supported interprocedural data-flow/taint relations to CodeQL. A triggered class requires direct method entry plus presumptive specialized use when available/reliable or a concrete non-silent fallback; no fixed multi-tool pipeline is introduced. CodeQL distinguishes local/external execution, separately executed GitHub-managed analysis, and the GitHub code-scanning result surface so result hosting is not mistaken for independent execution. Tool-specific mechanics and detailed convergence/cycle-economy rules are split behind deterministic direct routes to reduce loaded context without removing historical failure-mode defenses. GitHub, CodeQL, and live harness qualification remain optional unless project/task authority independently requires them.
